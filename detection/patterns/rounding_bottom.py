...
def detect(df: pd.DataFrame, stage: int, pivots: dict) -> Optional[dict]:
    ...
    for window in range(200, 99, -1):
        ...
        if coeffs[0] <= 0:          # must be concave up (U-shape)
            continue
    ...
    # ------------------------------------------------------------------ #
    # Resistance = left rim (close at start of window)                   #
    # Base low = minimum close in window                                  #
    # ------------------------------------------------------------------ #
    resistance = round(float(best["closes"][0]), 2)
    base_low   = float(best["closes"].min())
    ...
    sl_price = round(base_low * 0.98, 2)

    return {
        "pattern_key":      "rounding_bottom",
        "state":            state,
