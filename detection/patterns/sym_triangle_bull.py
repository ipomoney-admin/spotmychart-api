...
def _project_trendline(indices: list, prices: list, target_idx: int) -> float:
    """Project the OLS trendline to a given bar index."""
    if len(indices) < 2:
        return prices[-1]
    x = np.array(indices, dtype=float)
    y = np.array(prices, dtype=float)
...
def detect(df: pd.DataFrame, stage: int, pivots: dict) -> Optional[dict]:
    ...
    # ------------------------------------------------------------------ #
    # Triangle window: anchored at the earlier of last-2 peak/trough      #
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    # Resistance: upper trendline projected to current bar                 #
    # ------------------------------------------------------------------ #
    resistance    = round(_project_trendline(peak_indices, peak_prices, last), 2)
